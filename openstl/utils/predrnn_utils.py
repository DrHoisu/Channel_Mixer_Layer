...
def reserve_schedule_sampling_exp(itr, batch_size, args):
    T, img_channel = args.pre_seq_length, args.img_channel
    total_channel = args.total_channel + args.img_channel if args.fusion_method == 'mix' else args.total_channel
    img_height, img_width = args.in_shape
    ...
    ones = np.ones((img_height // args.patch_size,
                    ...
                    args.patch_size ** 2 * total_channel))
...
def reshape_patch(img_tensor, patch_size):
    ...
    if img_tensor.ndim == 4:
        batch_size, img_height, img_width, num_channels = img_tensor.shape
        a = torch.reshape(img_tensor, [batch_size,
                                img_height//patch_size, patch_size,
                                img_width//patch_size, patch_size,
                                num_channels])
        b = a.transpose(2, 3)
        patch_tensor = torch.reshape(b, [batch_size,
                                  img_height//patch_size,
                                  img_width//patch_size,
                                  patch_size*patch_size*num_channels])
    return patch_tensor
