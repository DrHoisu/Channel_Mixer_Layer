...
class BaseExperiment(object):
    ...
    def display_method_info(self):
        ...
        C = self.args.total_channel #+ self.args.img_channel if self.args.fusion_method == 'mix' else self.args.total_channel
        T = self.args.pre_seq_length
        H, W = self.args.in_shape
        ...
        elif self.args.method == 'fmn':
            if self.args.method_type in ['simvp', 'tau']:
                C = self.args.total_channel + self.args.img_channel if self.args.fusion_method == 'mix' else self.args.total_channel
                input_dummy = torch.ones(1, self.args.total_length, C, H, W).to(self.device)
            elif self.args.method_type == 'phydnet':
                _tmp_input1 = torch.ones(1, self.args.pre_seq_length, C, H, W).to(self.device)
                _tmp_input2 = torch.ones(1, self.args.aft_seq_length, C, H, W).to(self.device)
                _tmp_constraints = torch.zeros((49, 7, 7)).to(self.device)
                input_dummy = (_tmp_input1, _tmp_input2, _tmp_constraints)
            elif self.args.method_type in ['convlstm', 'predrnnpp', 'predrnn', 'mim', 'e3dlstm', 'mau']:
                Hp, Wp = H // self.args.patch_size, W // self.args.patch_size
                Cp = self.args.patch_size ** 2 * C
                _tmp_input = torch.ones(1, self.args.total_length, Hp, Wp, Cp).to(self.device)
                C = self.args.total_channel + self.args.img_channel if self.args.fusion_method == 'mix' else self.args.total_channel
                ...
                _tmp_flag = torch.ones(1, self.args.aft_seq_length - 1, Hp, Wp, Cp).to(self.device)
                input_dummy = (_tmp_input, _tmp_flag)
            elif self.args.method_type in ['swinlstm_d', 'swinlstm_b']:
                input_dummy = torch.ones(1, self.args.total_length, H, W, C).to(self.device)
            elif self.args.method_type == 'predrnnv2':
                Hp, Wp = H // self.args.patch_size, W // self.args.patch_size
                Cp = self.args.patch_size ** 2 * C
                _tmp_input = torch.ones(1, self.args.total_length, Hp, Wp, Cp).to(self.device)
                C = self.args.total_channel + self.args.img_channel if self.args.fusion_method == 'mix' else self.args.total_channel
        ...
        if self.args.method == 'fmn':
            if self.args.exp_stage == 'value_net_only':
                flops = FlopCountAnalysis(self.method.model['value_net'].to(self.device), input_dummy)
            else:
                flops = FlopCountAnalysis(self.method.model['index_net'].to(self.device), input_dummy)
        else:
