...
def MAE(pred, true, spatial_norm=False): #for iono dataset
    """
    # prediction accuracy for each time step
    ...
    print_log(" mae error: " + str(np.mean(np.abs(pred-true), axis=(0, 1)).mean() * 200))
    print_log(" pred mean: " + str(np.mean(np.abs(pred), axis=(0, 1)).mean() * 200))
    """
    if not spatial_norm:
        # for ionospheric total electron content prediction, average error at each position, with TECU unit after multiplying 200
        #return np.mean(np.abs(pred-true), axis=(0, 1)).mean() * 200
        # default error calculation in the research field of spatiotemporal predictive learning, sum of the whole image.
        return np.mean(np.abs(pred-true), axis=(0, 1)).sum()
    else:
        norm = pred.shape[-1] * pred.shape[-2] * pred.shape[-3]
        # for ionospheric total electron content prediction, average error at each position, with TECU unit after multiplying 200
        #return np.mean(np.abs(pred-true) / norm, axis=(0, 1)).mean() * 200
        # default error calculation in the research field of spatiotemporal predictive learning, sum of the whole image.
...
def RMSE(pred, true, spatial_norm=False): #for iono dataset
    '''
    for i in range(pred.shape[1]):
        if not spatial_norm:
    ...
    if not spatial_norm:
        # for ionospheric total electron content prediction, average error at each position, with TECU unit after multiplying 200
        #return np.sqrt(np.mean((pred-true)**2, axis=(0, 1)).mean()) * 200
        # default error calculation in the research field of spatiotemporal predictive learning, sum of the whole image.
        return np.sqrt(np.mean((pred-true)**2, axis=(0, 1)).sum())
    else:
        norm = pred.shape[-1] * pred.shape[-2] * pred.shape[-3]
        # for ionospheric total electron content prediction, average error at each position, with TECU unit after multiplying 200
        #return np.sqrt(np.mean((pred-true)**2 / norm, axis=(0, 1)).mean()) * 200
        # default error calculation in the research field of spatiotemporal predictive learning, sum of the whole image.
