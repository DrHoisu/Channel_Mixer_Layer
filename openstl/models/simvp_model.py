...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
                 ...
                 spatio_kernel_dec=3, act_inplace=True, pre_seq_length=10,
                 total_channel=1, img_channel=1, fusion_method=None, **kwargs):
        super(SimVP_Model, self).__init__()
        self.pre_seq_length = pre_seq_length
        C_out = img_channel
        self.C_out = C_out
        C = total_channel + img_channel if fusion_method == 'mix' else total_channel
        T = pre_seq_length
        H, W = in_shape  # T is pre_seq_length
        H, W = int(H / 2**(N_S/2)), int(W / 2**(N_S/2))  # downsample 1 / 2**(N_S/2)
        act_inplace = False
        self.enc = Encoder(C, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.dec = Decoder(hid_S, C_out, N_S, spatio_kernel_dec, act_inplace=act_inplace)
    ...
    def forward(self, x_raw, **kwargs):
        ...
        if T != self.pre_seq_length:
            x_raw = x_raw[:, :self.pre_seq_length]
