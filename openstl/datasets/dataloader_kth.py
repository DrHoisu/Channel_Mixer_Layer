...
class KTHDataset(Dataset):
    ...
    def __init__(self, datas, indices, pre_seq_length, aft_seq_length, use_augment=False, exp_stage=None):
    ...
    def __getitem__(self, i):
        ...
        if self.exp_stage == 'index_net_only':
            labels = labels.view(self.aft_seq_length, 1, imgs.shape[2]*imgs.shape[3])
            labels_value, labels_index = torch.sort(labels, dim=2)
            _, labels_index = torch.sort(labels_index, dim=2)
            if self.exp_stage == 'value_net_only':
                labels = labels_value.view(self.aft_seq_length, 1, imgs.shape[2], imgs.shape[3])
            else:
                #labels_index = torch.where(labels<0.5, torch.zeros_like(labels_index), labels_index)
                #labels_new = torch.gather(labels_value, dim=2, index= labels_index)
                #print(labels_new==labels)
                labels_index = labels_index.view(self.aft_seq_length, 1, imgs.shape[2], imgs.shape[3])
                labels = labels_index.to(dtype=torch.float32)/(imgs.shape[2]*imgs.shape[3])
            #output = input[self.n_frames_input:]
...
def load_data(batch_size, val_batch_size, data_root, num_workers=4,
              ...
              distributed=False, use_augment=False, use_prefetcher=False, drop_last=False, exp_stage=None):
    ...
    train_set = KTHDataset(train_input_handle.datas,
                           ...
                           pre_seq_length,
                           aft_seq_length, use_augment=use_augment, exp_stage=exp_stage)
    test_set = KTHDataset(test_input_handle.datas,
                          test_input_handle.indices,
                          pre_seq_length,
                          aft_seq_length, use_augment=False, exp_stage=exp_stage)
