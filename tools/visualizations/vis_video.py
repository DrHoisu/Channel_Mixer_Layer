...
def main():
    ...
    for method in method_list:
        ...
        elif 'iono' in args.dataname:
            #inputs = min_max_norm(inputs)
            #trues = min_max_norm(trues)
            #inputs_video = np.delete(inputs[idx, 0:ncols, ...], -1, axis=2)
            trues_video = np.delete(trues[idx, 0:ncols, ...], -1, axis=2)
            #inputs = show_weather_bench(inputs_video, src_img=None, cmap='plasma').transpose(0, 3, 1, 2) #plasma
            trues = show_weather_bench(trues_video, src_img=None, cmap='plasma').transpose(0, 3, 1, 2)   #plasma
        else:
    ...
    for i, method in enumerate(method_list):
        ...
        if 'weather' in args.dataname:
            ...
            #print(predicts_dict[method][idx, 0, ...])
            #print(trues_temp[idx, 0, ...])
            predicts_video = np.delete(predicts_dict[method][idx, 0:ncols, ...], -1, axis=2)
            preds = show_weather_bench(predicts_video, src_img=None, cmap='plasma', vis_channel=args.vis_channel)
            preds = preds.transpose(0, 3, 1, 2)
        else:
